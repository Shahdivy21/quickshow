      ...
      {!selectedMovie ? (
        ...
        <div className="mt-8 flex flex-col lg:flex-row gap-8 items-start">
          ...
          <div className="w-full lg:w-[70%] animate-in fade-in slide-in-from-right-4">
            <div className="bg-[#18191b] border border-gray-800 rounded-3xl p-6 lg:p-8 shadow-2xl">
              <div className="flex items-center gap-3 mb-6 pb-5 border-b border-gray-800/80">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">Add timings and theater mapping for {selectedMovieData?.title}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {/* Theater */}
                <div>
                  <label className="block text-[11px] font-bold text-gray-400 mb-2 uppercase tracking-wider">Select Theater</label>
                  <div className="relative group">
                    ...
                    <select
                      ...
                      className="w-full pl-11 pr-10 py-3.5 bg-black/40 text-sm text-gray-200 border border-gray-700 rounded-xl focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary appearance-none transition-all cursor-pointer hover:bg-black/60"
                    >
                      <option value="N/A" disabled className="text-gray-500">Select Theater Location</option>
                      {theaters.map((t, i) => (
                ...
                {/* Price */}
                <div>
                  <label className="block text-[11px] font-bold text-gray-400 mb-2 uppercase tracking-wider">Ticket Price</label>
                  <div className="relative group">
                    ...
                    <input
                      ...
                      className="w-full pl-9 pr-4 py-3.5 bg-black/40 text-sm text-gray-200 border border-gray-700 rounded-xl focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all hover:bg-black/60 placeholder:text-gray-600"
                      placeholder="0.00"
                    />
                  </div>
                </div>
              ...
              <div className="mt-8 border-t border-gray-800/80 pt-6">
                 <label className="block text-[11px] font-bold text-gray-400 mb-3 uppercase tracking-wider">Add Date & Time</label>
                 <div className="flex flex-col sm:flex-row gap-3">
                    <div className="flex-1 relative group">
                       ...
                       <input
                          ...
                          className="w-full pl-11 pr-4 py-3.5 bg-black/40 text-sm text-gray-200 border border-gray-700 rounded-xl focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all hover:bg-black/60"
                          style={{ colorScheme: "dark" }}
                       />
                    </div>
                    <button
                      onClick={handleDateTimeAdd}
                      className="bg-primary hover:bg-primary/90 text-white text-sm font-bold px-8 py-3.5 rounded-xl transition-all flex justify-center items-center gap-2 shadow-lg shadow-primary/20"
                    >
                      <Plus className="w-4 h-4" /> Add Slot
                    </button>
                 </div>
                 <div className="mt-8 pt-6 border-t border-gray-800/80">
                    <label className="block text-[11px] font-bold text-gray-400 mb-4 uppercase tracking-wider flex items-center gap-2">
                    ...
                    {/* Custom Time Input */}
                    <div className="flex gap-2 mb-6 p-2 bg-black/20 rounded-xl border border-gray-800/50">
                       <div className="relative flex-1 group">
                          <Clock className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-gray-500 group-focus-within:text-primary transition-colors" />
                          <input 
                             type="time" 
                             value={newQuickTime}
                             onChange={(e) => setNewQuickTime(e.target.value)}
                             className="w-full pl-9 pr-3 py-2.5 bg-gray-900/50 border border-gray-700 rounded-lg text-xs text-gray-200 focus:outline-none focus:border-primary transition-all"
                             style={{ colorScheme: "dark" }}
                          />
                       </div>
                       <button 
                          onClick={addQuickTime}
                          className="bg-gray-800 hover:bg-gray-700 text-white text-[10px] font-bold px-4 py-2.5 rounded-lg transition-all border border-gray-700 whitespace-nowrap"
                       >
                          + Add Custom
                       </button>
                    </div>
                    ...
                    {/* Selected Times List */}
                    {quickFillTimes.length > 0 && (
                       <div className="flex flex-wrap gap-2 mb-6">
                          {quickFillTimes.map((time) => (
                             <div key={time} className="flex items-center gap-1.5 bg-green-500/10 border border-green-500/20 pl-2.5 pr-1 py-1 rounded-md group">
                                <span className="text-[11px] font-bold text-green-500">{time}</span>
                                <button 
                                   onClick={() => removeQuickTime(time)}
                                   className="text-green-500/50 hover:text-red-400 p-0.5 rounded transition"
                                >
                    ...
                    <button
                       ...
                       className="w-full flex items-center justify-center gap-2 text-xs font-bold text-primary bg-primary/10 hover:bg-primary/20 border border-primary/20 px-6 py-3.5 rounded-xl transition-all shadow-sm"
                    >
                       ✨ Quick Fill Standard Schedule (10 Days, {quickFillTimes.length} Shows/Day)
                    </button>
                 </div>
              </div>
              ...
              <div className="mt-8 pt-6 border-t border-gray-800/80 flex justify-end">
                <button
                  ...
                  className="bg-green-600 hover:bg-green-500 text-white text-sm font-bold px-10 py-3.5 rounded-xl shadow-lg shadow-green-600/20 transition-all flex items-center gap-2 hover:-translate-y-0.5"
                >
